...
function ShapeProperties({ element, update }: PropertyGroupProps) {
  ...
  return <>
    <PropertySection title="Fill" open>
      ...
      {!gradientPaint ? <ColorRow label="Color" value={element.fill} onChange={(fill) => update({ fill, fillPaint: { type: 'solid', color: fill } })} /> : <><ColorRow label="Start" value={gradientPaint.stops[0].color} onChange={(color) => update({ fillPaint: { ...gradientPaint, stops: gradientPaint.stops.map((stop, index) => index ? stop : { ...stop, color }) } })} /><ColorRow label="End" value={gradientPaint.stops.at(-1)!.color} onChange={(color) => update({ fillPaint: { ...gradientPaint, stops: gradientPaint.stops.map((stop, index) => index === gradientPaint.stops.length - 1 ? { ...stop, color } : stop) } })} />{gradientPaint.type === 'linear-gradient' && <RangeRow label="Angle" value={gradientPaint.angle} min={0} max={360} onChange={(angle, live) => editor.updateElement(element.id, { fillPaint: { ...gradientPaint, angle } }, live)} />}</>}
    </PropertySection>
...
function EffectsProperties({ element, update }: PropertyGroupProps) {
  ...
  return <PropertySection title="Effects">
    ...
    <label className="select-control add-effect"><span>Add effect</span><select value="" onChange={(event) => { if (event.target.value) add(event.target.value as ElementEffect['type']); event.target.value = '' }}><option value="">Choose…</option><option value="drop-shadow">Drop shadow</option><option value="outline">Outline</option><option value="glow">Glow</option><option value="gaussian-blur">Gaussian blur</option></select><ChevronDown size={13} /></label>
  </PropertySection>
}
...
function EffectEditor({ element, effect, update }: { element: EdonElement; effect: ElementEffect; update: PropertyGroupProps['update'] }) {
  ...
  return <div className="effect-card"><div><button className={`effect-toggle ${effect.enabled ? 'is-active' : ''}`} onClick={() => patchEffect({ enabled: !effect.enabled })} /><strong>{formatLabel(effect.type)}</strong><button onClick={() => update({ effects: element.effects.filter((item) => item.id !== effect.id) })}><X size={12} /></button></div>{effect.type === 'gaussian-blur' ? <RangeRow label="Radius" value={effect.radius} min={0} max={80} onChange={(radius, live) => patchEffect({ radius }, live)} /> : <>{'color' in effect && <ColorRow label="Color" value={effect.color} onChange={(color) => patchEffect({ color })} />}{effect.type === 'outline' && <RangeRow label="Width" value={effect.width} min={0} max={30} onChange={(width, live) => patchEffect({ width }, live)} />}{effect.type === 'glow' && <RangeRow label="Blur" value={effect.blur} min={0} max={80} onChange={(blur, live) => patchEffect({ blur }, live)} />}{effect.type === 'drop-shadow' && <><RangeRow label="Blur" value={effect.blur} min={0} max={80} onChange={(blur, live) => patchEffect({ blur }, live)} /><div className="property-grid"><NumberField label="X" value={effect.offsetX} onChange={(offsetX) => patchEffect({ offsetX })} /><NumberField label="Y" value={effect.offsetY} onChange={(offsetY) => patchEffect({ offsetY })} /></div></>}</>}</div>
}
...
function ColorRow({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="color-control"><span className="color-swatch checkerboard"><i style={{ background: value }} /></span><span>{label}</span><input type="color" value={toColorValue(value)} onChange={(event) => onChange(event.target.value)} /><code>{value.toUpperCase()}</code></label> }
