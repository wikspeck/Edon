...
function EffectEditor({ element, effect, update }: { element: EdonElement; effect: ElementEffect; update: PropertyGroupProps['update'] }) {
  ...
  return <div className="effect-card">
    ...
    {effect.type === 'gaussian-blur' ? <RangeRow label="Radius" value={effect.radius} min={0} max={80} onChange={(radius, live) => patchEffect({ radius }, live)} /> : <>
      ...
      {effect.type === 'outline' && <><RangeRow label="Thickness" value={effect.width} min={0} max={30} onChange={(width, live) => patchEffect({ width }, live)} /><RangeRow label="Opacity" value={Math.round(effect.opacity * 100)} min={0} max={100} onChange={(opacity, live) => patchEffect({ opacity: opacity / 100 }, live)} /></>}
      {effect.type === 'glow' && <><RangeRow label="Intensity" value={effect.strength} min={.25} max={3} step={.25} onChange={(strength, live) => patchEffect({ strength }, live)} /><RangeRow label="Radius" value={effect.blur} min={0} max={80} onChange={(blur, live) => patchEffect({ blur }, live)} /><RangeRow label="Spread" value={effect.spread} min={0} max={24} onChange={(spread, live) => patchEffect({ spread }, live)} /><RangeRow label="Opacity" value={Math.round(effect.opacity * 100)} min={0} max={100} onChange={(opacity, live) => patchEffect({ opacity: opacity / 100 }, live)} /></>}
      {effect.type === 'drop-shadow' && <><RangeRow label="Opacity" value={Math.round(effect.opacity * 100)} min={0} max={100} onChange={(opacity, live) => patchEffect({ opacity: opacity / 100 }, live)} /><RangeRow label="Blur" value={effect.blur} min={0} max={80} onChange={(blur, live) => patchEffect({ blur }, live)} /><RangeRow label="Spread" value={effect.spread} min={0} max={24} onChange={(spread, live) => patchEffect({ spread }, live)} /><div className="property-grid"><NumberField label="X" value={effect.offsetX} onChange={(offsetX) => patchEffect({ offsetX })} /><NumberField label="Y" value={effect.offsetY} onChange={(offsetY) => patchEffect({ offsetY })} /></div></>}
