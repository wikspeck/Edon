...
export async function createRasterStroke(page: EdonPage, existing: EdonElement | null, points: VectorPoint[], settings: ArtToolSettings, erase = false): Promise<EdonElement> {
  const canvas = document.createElement('canvas'); canvas.width = page.width; canvas.height = page.height
  const context = canvas.getContext('2d', { willReadFrequently: true })!
  if (existing?.imageUrl) context.drawImage(await loadImage(existing.imageUrl), 0, 0, page.width, page.height)
  ...
  const element = existing ? { ...existing } : createElement('raster', 0, 0, page.width, page.height)
  element.name = existing?.name ?? (erase ? 'Erased raster' : 'Raster paint')
  element.imageUrl = canvas.toDataURL('image/png')
...
export async function paintEnclosedRegion(page: EdonPage, point: VectorPoint, settings: ArtToolSettings): Promise<BucketResult> {
  ...
  await renderElements(context, page.elements.filter((element) => element.visible), false)
  ...
  const element = createElement('raster', 0, 0, page.width, page.height); element.name = 'Paint bucket fill'; element.imageUrl = output.toDataURL('image/png')
...
export async function sampleVisibleColor(page: EdonPage, point: VectorPoint): Promise<string> {
  const canvas = document.createElement('canvas'); canvas.width = page.width; canvas.height = page.height
  const context = canvas.getContext('2d', { willReadFrequently: true })!
  await renderElements(context, page.elements.filter((element) => element.visible), true)
