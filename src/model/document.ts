...
export interface EdonDocument { version: 2; id: string; name: string; createdAt: string; updatedAt: string; activePageId: string; pages: EdonPage[] }
...
export function createDocument(name: string, width: number, height: number): EdonDocument {
  ...
  return { version: 2, id: createId('doc'), name: name.trim() || 'Untitled', createdAt: timestamp, updatedAt: timestamp, activePageId: pageId, pages: [{ id: pageId, name: 'Page 1', width, height, background: '#ffffff', elements: [] }] }
}
...
export function createElement(type: ElementType, x: number, y: number, width?: number, height?: number): EdonElement {
  ...
  return {
    ...
    stroke: type === 'frame' ? '#a0a0a8' : isLine ? '#2d2d31' : '#00000000', strokeWidth: type === 'frame' ? 1 : isLine ? 3 : 0, strokeDash: [],
...
export function normalizeElement(element: Partial<EdonElement> & Pick<EdonElement, 'id' | 'type' | 'name' | 'x' | 'y' | 'width' | 'height'>): EdonElement {
  ...
  return { ...base, ...element, parentId: element.parentId ?? null, scaleX: element.scaleX ?? 1, scaleY: element.scaleY ?? 1, blendMode: element.blendMode ?? 'normal', fillPaint: element.fillPaint ?? solidPaint(element.fill ?? base.fill), strokeDash: element.strokeDash ?? [], cornerRadii: element.cornerRadii ?? [radius, radius, radius, radius], effects: element.effects ?? [], adjustments: element.type === 'image' ? { ...DEFAULT_ADJUSTMENTS, ...element.adjustments } : element.adjustments, crop: element.type === 'image' ? element.crop ?? { x: 0, y: 0, width: 1, height: 1 } : element.crop }
}
...
export function migrateDocument(input: unknown): EdonDocument | null {
  ...
  if (document.version === 2) return { ...(document as EdonDocument), pages: document.pages.map((page) => ({ ...page, elements: page.elements.map(normalizeElement) })) }
  return { ...(document as Omit<EdonDocument, 'version' | 'pages'>), version: 2, pages: document.pages.map((page) => ({ ...page, elements: page.elements.map(normalizeElement) })) }
}
